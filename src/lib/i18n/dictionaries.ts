    ...
    items: [
      {
        ...
        approach: "基本遊戲鋪上呼吸感的 pad、低頻耳語與緩慢心跳節拍——夠安靜以隱身於轉軸音效之後。Free Game 切換到具推進感的合唱與調變鈴聲，傳達獎勵但不脫離鬼魅基調。中獎音效設計為基底氛圍的解析，而非脫離。",
      ...
      },
      {
        ...
        approach: "基本遊戲以明亮嗩吶、笛子旋律疊在 130 BPM 的推進打擊樂上（太鼓 + 響棒）。裝飾音以磁帶飽和輕度處理柔化高頻。Free Game 模式拉開混音空間，加入完整弦樂群與合唱命中音。中獎音效採用調至音樂主調的鈴聲層，讓每次中獎都「在調上」。",
      ...
      },
      {
        ...
        approach: "我們建立了一套 5 層音效堆疊系統：每次成功連鎖會加入一層樂器（卡林巴 → 弦樂 → 銅管 → 合唱 → 完整合奏），加上一個調性對應主旋律的高音中獎音效。當連鎖中斷時，音層會優雅地折回基底——不會突兀切斷。中獎主要音效保留給最終結算，因此每次中獎都像真正的回報。",
