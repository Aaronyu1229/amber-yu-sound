...
const en: Dictionary = {
  ...
  portfolio: {
    ...
    items: [
      ...
      { type: "SLOT", services: "Music & Sound Design", tags: ["Japanese", "Lucky Cat", "Bonus Mechanics"] },
  ...
  portfolioDetail: {
    ...
    items: [
      ...
      },
      {
        ...
        clientRole: "Japanese-themed slot built around daruma-styled lucky cats, a three-tier bonus system (Boost / Multiply / Extra), and a lock-and-key mechanic that escalates feature triggers.",
        genre: "Japanese Theme / Bonus Mechanics / Slot",
        year: "2025",
        platform: "Mobile & Web",
        duration: "6 weeks",
        overview: "Lock-Key Meow Meow leans into Japanese folk imagery — daruma-styled lucky cats, sakura petals, paper-lantern temples — paired with a layered three-tier bonus system. The audio brief asked for a sonic identity that reads as unmistakably Japanese without leaning on the most-overused tropes, with each bonus tier carrying its own distinct character.",
        challenge: "Three separate bonus modes (Boost, Multiply, Extra) all share the same base game, so the audio system had to make each one instantly recognisable while still feeling like a single coherent world. We also needed the lock-and-key meta-mechanic to land sonically — every key drop and lock release had to feel satisfying.",
        approach: "The base game is grounded in koto, shamisen, and tuned suzu bells over a soft taiko pulse. Each bonus tier substitutes a signature instrument and tempo — Boost rides higher koto runs at a brisker tempo, Multiply leans into ensemble taiko hits with brass swells, Extra opens up the soundstage with full string and choir overlays. Lock-and-key SFX use layered metal (key tumblers, latch release) with a bell sweetener so the unlock always reads as 'reward'.",
        result: "The three-tier bonus audio mapped cleanly onto the studio's progression dashboard and was reused as a template for a follow-up title using the same lock-and-key mechanic with a different theme.",
        assets: [
          { label: "Music Tracks", count: "4" },
          { label: "Bonus Stingers", count: "3" },
          { label: "Sound Effects", count: "85+" },
          { label: "Win Stingers", count: "12" },
        ],
...
const zh: Dictionary = {
  ...
  portfolio: {
    ...
    items: [
      ...
      { type: "老虎機", services: "音樂與音效設計", tags: ["日式風格", "招財貓", "獎勵機制"] },
  ...
  portfolioDetail: {
    ...
    items: [
      ...
      },
      {
        ...
        clientRole: "日式主題老虎機，圍繞達摩造型的招財貓符號、三層獎勵系統（Boost / Multiply / Extra）、以及推升特殊功能觸發的鎖鑰機制。",
        genre: "日式主題 / 獎勵機制 / 老虎機",
        year: "2025",
        platform: "手機 & 網頁",
        duration: "6 週",
        overview: "Lock-Key Meow Meow 深植於日本民俗意象——達摩造型的招財貓、櫻花花瓣、紙燈籠寺院——並搭配多層次的三層獎勵系統。音效需求是要打造出明顯日式但不落俗套的聲音識別，而且每一層獎勵都要有自己獨特的個性。",
        challenge: "三種獨立的獎勵模式（Boost、Multiply、Extra）共用同一個主遊戲，所以音效系統必須讓每一種模式立即可辨識，同時又像是同一個世界的延伸。鎖鑰元機制也得在聽覺上落地——每一次鑰匙落下、每一次鎖打開都要讓人感覺到滿足。",
        approach: "主遊戲以古箏、三味線、調音的鈴鐺鋪在柔和的太鼓脈動上。每一層獎勵置換不同的代表性樂器與速度——Boost 用更快速度的高音古箏走句，Multiply 強調合奏太鼓擊點加上銅管漸強，Extra 拉開聲場加入完整弦樂與合唱層。鎖鑰音效採用多層金屬聲（鑰匙轉動、門栓彈開）搭配鈴鐺裝飾音，讓每一次解鎖都像「獎勵」。",
        result: "三層獎勵的音效對應乾淨對齊工作室的進度儀表板，並被改編成同主題不同樣貌的續作模板。",
        assets: [
          { label: "音樂曲目", count: "4" },
          { label: "獎勵音效", count: "3" },
          { label: "音效", count: "85+" },
